...
const absPath = {
    ...
    artifacts: ( ...relPath ) => path.resolve(absPath.game('.artifacts'), ...relPath),
    tmp: ( ...relPath ) => path.resolve(absPath.engine('tmp'), ...relPath ),
    artifactsIn: ( ...relPath ) => path.resolve(currArtifact, 'in', ...relPath),
    artifactsOut: ( ...relPath ) => path.resolve(currArtifact, 'out', ...relPath),
    setCurrDist: path => {
        currDist = path
    },
    setCurrArtifact: path => {
        currArtifact = path
    },
    make: path.resolve,
